...
import { getVoiceProfile, enrolAndSave } from "@/lib/voice-profile";
...
export async function POST(req: Request) {
  ...
  const existing = await getVoiceProfile(session.user.id);
  if (existing) {
    return NextResponse.json(
      { error: "Voice profile already exists. Use re-enrol to replace it." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    console.error("Voice enrolment failed", error);
