...
import { getNotionAuthorizeUrl } from "@/lib/integrations/notion";
...
export async function GET(req: Request) {
  ...
  if (!process.env.NOTION_CLIENT_ID) {
    return NextResponse.json(
      { error: "Notion is not configured on this server yet" },
      { status: 503 }
    );
  }

  return NextResponse.redirect(getNotionAuthorizeUrl(workspaceId));
}
